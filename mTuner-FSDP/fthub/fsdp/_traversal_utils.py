...
``import fthub.fsdp._traversal_utils`` and not like
...
from fthub.fsdp._common_utils import _FSDPState, _get_module_fsdp_state
...
def _composable(module: nn.Module) -> bool:
    ...
    out = _get_registry(module)
    if out is None:
        return True
    return "replicate" not in out
