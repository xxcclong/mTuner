...
from ._comm_utils import (
    get_start_end,
    device_aware_communication,
)
import logging
import time
...
class RunMemTrace:
    def __init__(self) -> None:
        self.unsharded_param = -1
        self.activation = -1
        self.pre_forward = -1

    def __str__(self):
        return f"RunMemTrace(unsharded {self.unsharded_param} activation {self.activation} pre_forward {self.pre_forward})"
...
class FlatParamHandle:
    ...
    global_handle_init_id = 0
    global_handle_visit_id = 0
    memory_traces = []
    ...
    def __init__(
        ...
        self.debug = False
        # Extra handle information
        self.handle_init_id = FlatParamHandle.global_handle_init_id
        FlatParamHandle.global_handle_init_id += 1
        self.handle_visit_id = -1
        # Profile memory
        self.profile = False
        self.profile_results = {}
        self.batch_size = -1
        self.curr_profile_results = None
        self.memory_state = {}
        self.param_size = 0
        self.replica_config = replica_config
        self.skip_comm = False
        if replica_config is not None:
            self.intra_weight_replica_rate = replica_config.intra_weight_replica_rate
            self.intra_weight_replica_strategy = (
                replica_config.intra_weight_replica_strategy
            )
            # self.inter_weight_replica_layer_id = (
            #     replica_config.config.inter_weight.replica_layer_id
            # )
            # logging.info(
            #     f"FlatParamHandle {self.handle_init_id} {self.intra_weight_replica_rate} {self.intra_weight_replica_strategy} {self.skip_comm}"
            # )
            # if self.flat_param.requires_grad:
            #     self.intra_weight_replica_rate = 0

            # self.intra_weight_replica_strategy = intra_weight_replica_strategy
            self.intra_weight_replica_size = -1
            # Inter-weight replication
            self.time_dim_layer_id = replica_config.config.time_dim.layer_id
            self.need_ac = replica_config.config.ac == "checkpoint"
        else:
            self.intra_weight_replica_rate = 0
            self.intra_weight_replica_strategy = "homo"
            self.intra_weight_replica_size = -1
            self.time_dim_layer_id = -1
            self.need_ac = False
        self.unshard_cnt = 0
        self.unshard_allgather = 0
        self.allgather_size = 0
        self.repeated = replica_config.config.repeated
        self.partial_unshard = None
        if self.repeated:
            self.num_repeat = (
                replica_config.config.train.batch_size
                // replica_config.config.dual_model.batch_size
            )
        else:
            self.num_repeat = 0

    def set_profile(self, batch_size):
        # assert batch_size not in self.profile_results
        self.profile_results[batch_size] = RunMemTrace()
        self.profile = True
        self.curr_profile_results = self.profile_results[batch_size]

    def skip_communication(self):
        self.skip_comm = True

    def set_time_measurement(self):
        self.start_events = []
        self.stop_events = []
    ...
    def get_time_measurement(self):
        ...
        for i in range(len(self.start_events)):
            t += self.start_events[i].elapsed_time(self.stop_events[i])
        return t

    def finish_profile(self):
        self.profile = False

    def prof_memory_pre_forward(self):
        if not self.profile:
            return
        self.curr_profile_results.pre_forward = torch.cuda.memory_allocated(self.device)

    def prof_memory_post_forward(self):
        if not self.profile:
            return
        self.curr_profile_results.activation = (
            torch.cuda.memory_allocated(self.device)
            - self.curr_profile_results.pre_forward
        )
    ...
    def __str__(self):
        ...
        for s in self.flat_param._shapes:
            num_param += s.numel()
        return f"FlatParamHandle({self.handle_init_id} {self.flat_param.device}+{self.flat_param.requires_grad}+{self._fully_sharded_module}+{self.flat_param._shapes} {num_param})"
    ...
    def _init_flat_param(
        ...
        for submodule_name, submodule in module.named_modules():
            for param_name, param in submodule.named_parameters(recurse=False):
                # logging.info(f"{param_name}, {param.shape}, {param.dtype}")
                self.names.append(module_name + "." + submodule_name + "." + param_name)
        ...
        # logging.info("----")
        # print("before", type(params_to_flatten))
        self.flat_param = FlatParamHandle.flatten_params(
            params_to_flatten, requires_grad
        )
        # print("after", type(params_to_flatten), type(self.flat_param))
    ...
    def shard(self):
        ...
        if not self.uses_sharded_strategy:
            ...
            # hkz-comment: Initial shard parameter here
            if self.intra_weight_replica_rate <= 0 or self.flat_param.requires_grad:
            ...
            elif self.intra_weight_replica_strategy == "hetero":
                # hkz-comment: has replica with rate r, each storing 1/world_size+r
                "目前的r就是1/world_size+r"
                (
                    sharded_flat_param,
                    new_replica_size,
                    length,
                ) = FlatParamHandle._get_shard_with_replica_hetero(
                    flat_param,
                    self.rank,
                    self.world_size,
                    self.intra_weight_replica_rate,
                )
                # print("shard: sharded_flat_param", sharded_flat_param)
                self.param_size = length
                self.intra_weight_replica_size = 1
                # self.flat_param.replica = 0
                flat_param.set_(sharded_flat_param)

                start, end = get_start_end(
                    length, new_replica_size, self.rank, self.world_size
                )

                self._init_shard_metadata(
                    0, start, end
                )  # TODO-hkz: should init meta data also for replica
                self.flat_param.replica = None
            elif self.intra_weight_replica_strategy == "homo":
                (
                    sharded_flat_param,
                    numel_padded,
                    replica,
                ) = FlatParamHandle._get_shard_with_replica(
                    flat_param,
                    self.rank,
                    self.world_size,
                    self.intra_weight_replica_rate,
                )
                size = self.flat_param.numel()
                self.intra_weight_replica_size = replica.numel()
                flat_param.set_(sharded_flat_param)
                start = sharded_flat_param.numel() * self.rank
                end = sharded_flat_param.numel() * (self.rank + 1) - 1  # inclusive
                self._init_shard_metadata(
                    numel_padded, start, end
                )  # TODO-hkz: should init meta data also for replica
                # Important-hkz: replica might be freed
                self.flat_param.replica = torch.empty_like(replica)
                self.flat_param.replica.copy_(replica)
    ...
    # Naive strategy: Same replication
    @staticmethod
    def _get_unpadded_shard_with_replica(
        tensor: Tensor,
        rank: int,
        world_size: int,
        rate: float,
        ...
        replica_size = math.ceil(flattened_tensor.shape[0] * rate)
        replica_size = (replica_size + world_size - 1) // world_size * world_size
        per_shard_size = (flattened_tensor.shape[0] - replica_size) // world_size
        per_shard_size = (per_shard_size + 8 - 1) // 8 * 8
        replica_size = flattened_tensor.shape[0] - per_shard_size * world_size
        p_assert(
            replica_size < flattened_tensor.shape[0] and replica_size >= 0,
            f"replica size {replica_size} {flattened_tensor.shape[0]}",
        )
        if replica_size > 0:
            replica = flattened_tensor[-replica_size:]
            tensor_to_shard = flattened_tensor[:-replica_size]
            size_per_chunk = tensor_to_shard.shape[0] // world_size
            chunk = tensor_to_shard[rank * size_per_chunk : (rank + 1) * size_per_chunk]
            numel_to_pad = 0
        else:
            chunks = torch.flatten(tensor).chunk(world_size)
            if len(chunks) < (rank + 1):
            ...
            replica = torch.empty(0, device=tensor.device)

        return chunk, numel_to_pad, replica

    @staticmethod
    def _get_shard_with_replica(
        tensor: Tensor, rank: int, world_size: int, rate: float
    ) -> Tuple[Tensor, int]:
        chunk, numel_to_pad, replica = FlatParamHandle._get_unpadded_shard_with_replica(
            tensor, rank, world_size, rate
        )
        shard = chunk.clone()
        if numel_to_pad > 0:
            shard = F.pad(shard, [0, numel_to_pad])
        return shard, numel_to_pad, replica

    #  strategy: different replication strategy
    @staticmethod
    def _get_shard_with_replica_hetero(
        tensor: Tensor,
        rank: int,
        world_size: int,
        rate: float,
        ...
        replica_size = math.ceil(flattened_tensor.shape[0] * rate)
        replica_size = (replica_size + world_size - 1) // world_size * world_size
        new_replica_size = int((length - replica_size) // world_size + replica_size)
        new_replica_size = (new_replica_size + 8 - 1) // 8 * 8
        # print(new_replica_size)
        p_assert(new_replica_size < length, f"replica size {new_replica_size} {length}")

        comm_size = length - new_replica_size

        p_assert(world_size % 2 == 0, f"world size {world_size}")
        if world_size % 2 == 0:
            if comm_size <= new_replica_size:
                """2,4,6,8都可行"""
                if rank % 2 == 0:
                    chunk = flattened_tensor[:new_replica_size]
                else:
                    chunk = flattened_tensor[-new_replica_size:]
            elif comm_size <= 3 * new_replica_size and (
                world_size == 4 or world_size == 8
            ):
                """4,8可行"""
                ...
                if rank % 4 == 0:
                    ...
                    chunk = flattened_tensor[l3 : l3 + new_replica_size]
                elif rank % 4 == 3:
                    chunk = flattened_tensor[
                ...
                if rank == 0:
                    ...
                    chunk = flattened_tensor[l5 : l5 + new_replica_size]
                elif rank == 5:
                    chunk = flattened_tensor[
                        length - l6 - new_replica_size : length - l6
                    ]
            elif world_size == 6:
                ...
                if rank == 0:
                    ...
                    chunk = flattened_tensor[
                        new_replica_size
                        - (new_replica_size - l3) // 2 : 2 * new_replica_size
                        - (new_replica_size - l3) // 2
                    ]
                elif rank == 5:
                    chunk = flattened_tensor[
                        3 * new_replica_size
                        + l3
                        - (new_replica_size - l4) // 2 : 4 * new_replica_size
                        + l3
                        - (new_replica_size - l4) // 2
                    ]
            elif world_size == 8:
                l3 = (length - 4 * new_replica_size) // 2
                l4 = length - 4 * new_replica_size - l3
                ...
                if rank == 0:
                    ...
                    chunk = flattened_tensor[l5 : new_replica_size + l5]
                elif rank == 5:
                    chunk = flattened_tensor[
        ...
        shard = chunk.clone()
        return shard, new_replica_size, length
    ...
    def init_flat_param_attributes(self) -> None:
        ...
        if self.uses_sharded_strategy:
            ...
            if self.intra_weight_replica_rate <= 0 or self.flat_param.requires_grad:
                ...
                assert self.intra_weight_replica_size >= 0
                padded_unsharded_numel = (
                    flat_param.numel() * self.world_size
                    + self.intra_weight_replica_size
                )
            elif self.intra_weight_replica_strategy == "hetero":
                padded_unsharded_numel = self.param_size
    ...
    def unshard(self):
        ...
        """
        # logging.info(f"unshard {self.unshard_cnt} {self._training_state}")
        if self.profile and self._training_state == HandleTrainingState.FORWARD:
            before_memory = torch.cuda.memory_allocated(self.device)
        if self.debug:
            logging.info(
                f"before unshard {self.flat_param.device} {self.flat_param.requires_grad} {self.handle_init_id} {self._training_state} {torch.cuda.memory_allocated(self.device)}"
            )
            # print(self.flat_param.shape, self._get_padded_unsharded_flat_param().shape)
        self.log_memory_trace()
        if self.profile:
            self.memory_state[f"start_unshard_{self._training_state}"] = (
                torch.cuda.memory_allocated(self.device)
            )

        if self.handle_visit_id == -1:
            self.handle_visit_id = FlatParamHandle.global_handle_visit_id
            FlatParamHandle.global_handle_visit_id += 1
        ...
        if not self.needs_unshard():
            ...
            if self.debug:
                logging.info(
                    f"(no need unshard) after unshard {self.flat_param.device} {self.flat_param.requires_grad} {self.handle_init_id} {self._training_state} {torch.cuda.memory_allocated(self.device)}"
                )
            if self.profile:
                self.memory_state[f"end_unshard_{self._training_state}"] = (
                    torch.cuda.memory_allocated(self.device)
                )
            self.log_memory_trace()
            return
        ...
        self.unshard_allgather += 1
        if self.intra_weight_replica_size <= 0 or self.flat_param.requires_grad:
            padded_unsharded_flat_param = self._all_gather_flat_param(
                unsharded_flat_param
            )
        else:
            padded_unsharded_flat_param = self._all_gather_flat_param_with_replica(
                unsharded_flat_param
            )
        self.allgather_size += padded_unsharded_flat_param.numel()
        self._use_unsharded_flat_param(padded_unsharded_flat_param)
        if self.debug:
            logging.info(
                f"after unshard {self.flat_param.device} {self.flat_param.requires_grad} {self.handle_init_id} {self._training_state} {torch.cuda.memory_allocated(self.device)}"
            )
        if self.profile:
            self.memory_state[f"end_unshard_{self._training_state}"] = (
                torch.cuda.memory_allocated(self.device)
        ...
        self.log_memory_trace()
        if self.profile and self._training_state == HandleTrainingState.FORWARD:
            self.curr_profile_results.unsharded_param = (
                torch.cuda.memory_allocated(self.device) - before_memory
            )
    ...
    def _run_all_gather(self, dst, src, pg):
        if self.skip_comm:
            return
        # torch.cuda.synchronize(self.device)
        # if self.rank == 0:
        #     print(f"allgather-size {src.numel()}")
        # e1 = torch.cuda.Event(enable_timing=True)
        # e2 = torch.cuda.Event(enable_timing=True)
        # e1.record()
        dist.all_gather_into_tensor(dst, src, pg)
        # e2.record()
        # self.start_events.append(e1)
        # self.stop_events.append(e2)
    ...
    def _all_gather_flat_param(
        ...
        if self.partial_unshard is None:
            self._run_all_gather(
            ...
            target_place0 = (
                padded_unsharded_flat_param[: expected_numel // 2]
                if self.rank < 4
                else padded_unsharded_flat_param[expected_numel // 2 :]
            )
            target_place1 = (
                padded_unsharded_flat_param[expected_numel // 2 :]
                if self.rank < 4
            ...
            self._run_all_gather(
                target_place0,
                sharded_flat_param,
                self.partial_process_group,
            )
            self._run_all_gather(
                target_place1,
                self.partial_unshard,
                self.partial_process_group,
            )
            self.partial_unshard = None
    ...
    def _all_gather_flat_param_with_replica(
        self,
        padded_unsharded_flat_param: Tensor,
    ) -> Tensor:
        p_assert(
        ...
        if self.intra_weight_replica_strategy == "hetero":
            sharded_flat_param = self.flat_param.data
            expected_numel = self.param_size
            p_assert(
                padded_unsharded_flat_param.numel() == expected_numel,
                f"Expects {expected_numel} numel but got {padded_unsharded_flat_param.numel()}, replica is {self.intra_weight_replica_size}",
            )
        else:
            sharded_flat_param = self.flat_param.data  # unique part
            expected_numel = (
                sharded_flat_param.numel() * self.world_size
                + self.intra_weight_replica_size
            )
            p_assert(
                padded_unsharded_flat_param.numel() == expected_numel,
        ...
        if self.intra_weight_replica_strategy == "homo":
            narrow_tensor = padded_unsharded_flat_param.narrow(
                0, 0, expected_numel - self.intra_weight_replica_size
            )
            self._run_all_gather(
                narrow_tensor,
                sharded_flat_param,
            ...
            padded_unsharded_flat_param[
                (expected_numel - self.intra_weight_replica_size) :
            ] = self.flat_param.replica
        elif self.intra_weight_replica_strategy == "hetero":
            world_size = self.world_size
            global_rank = self.rank
            new_replica_size = sharded_flat_param.numel()
            ...
            assert False, f"Unknown intra weight strategy {self.intra_weight_strategy}"

        return padded_unsharded_flat_param

    @staticmethod
    def generate_nodes(self, rank, n):
        A = []
        B = []
        for j in range(n):
            if rank != j:
    ...
    @staticmethod
    def exchange(self, tensor_a, replica_a, device_b_rank):
        "把a的replica发送到b, 同时从b接收参数"
        send_op = dist.P2POp(dist.isend, replica_a, device_b_rank)
        self.requests.append(send_op)
        recv_op = dist.P2POp(dist.irecv, tensor_a, device_b_rank)
        self.requests.append(recv_op)

    @staticmethod
    def send(self, replica_a, device_b_rank):
        "把device a上的参数发送给device b"
        send_op = dist.P2POp(dist.isend, replica_a, device_b_rank)
        self.requests.append(send_op)

    @staticmethod
    def receive(self, tensor_a, device_b_rank):
        "device a接收来自device b的参数"
        recv_op = dist.P2POp(dist.irecv, tensor_a, device_b_rank)
        self.requests.append(recv_op)
    ...
    def unshard_grad(self):
        ...
        if self.debug:
            logging.info(
                f"before unshard_grad {self.flat_param.device} {self.flat_param.requires_grad} {self.handle_init_id} {self._training_state} {torch.cuda.memory_allocated(self.device)}"
            )
        if not self.uses_sharded_strategy:
            self._use_unsharded_grad_views()
            if self.debug:
                logging.info(
                    f"after unshard_grad0 {self.flat_param.device} {self.flat_param.requires_grad} {self.handle_init_id} {self._training_state} {torch.cuda.memory_allocated(self.device)}"
                )
            return
        flat_param = self.flat_param
        ...
        if num_grad_none[0] == self.world_size:
            ...
            if self.debug:
                logging.info(
                    f"after unshard_grad1 {self.flat_param.device} {self.flat_param.requires_grad} {self.handle_init_id} {self._training_state} {torch.cuda.memory_allocated(self.device)}"
                )
            return
        ...
        self._run_all_gather(padded_unsharded_grad, sharded_grad, self.process_group)
        unsharded_size = self.flat_param._unpadded_unsharded_size
        ...
        if self.debug:
            logging.info(
                f"after unshard_grad2 {self.flat_param.device} {self.flat_param.requires_grad} {self.handle_init_id} {self._training_state} {torch.cuda.memory_allocated(self.device)}"
            )

    def reshard_grad(self):
        if self.debug:
            logging.info(
                f"before reshard_grad {self.flat_param.device} {self.flat_param.requires_grad} {self.handle_init_id} {self._training_state} {torch.cuda.memory_allocated(self.device)}"
            )
        if self._use_orig_params:
        ...
        if not self.uses_sharded_strategy:
            if self.debug:
                logging.info(
                    f"after reshard_grad0 {self.flat_param.device} {self.flat_param.requires_grad} {self.handle_init_id} {self._training_state} {torch.cuda.memory_allocated(self.device)}"
                )
            return
        ...
        if self.debug:
            logging.info(
                f"after reshard_grad1 {self.flat_param.device} {self.flat_param.requires_grad} {self.handle_init_id} {self._training_state} {torch.cuda.memory_allocated(self.device)}"
            )
    ...
    def reshard(self, free_unsharded_flat_param: bool):
        ...
        if self.debug:
            logging.info(
                f"before reshard {self.flat_param.device} {self.flat_param.requires_grad} {self.handle_init_id} {self._training_state} {torch.cuda.memory_allocated(self.device)}"
            )
        self.log_memory_trace()
        if self.profile:
            self.memory_state[f"start_reshard_{self._training_state}"] = (
                torch.cuda.memory_allocated(self.device)
            )
        is_forward = self._training_state == HandleTrainingState.FORWARD
        is_frozen = not self.flat_param.requires_grad
        self.time_dim_skip = (
            (self.time_dim_layer_id != -1)
            and self.handle_visit_id != -1
            and (self.handle_visit_id <= self.time_dim_layer_id)
            and (
                self._training_state
        ...
        # self.ac_skip = (
        #     self.need_ac
        #     and ((self.unshard_cnt - 1) % 3) == 1
        #     and not self.repeated
        #     and is_frozen
        #     and self._training_state != HandleTrainingState.IDLE
        # )
        # self.ac_skip = False
        # logging.info(f"hkz {self.need_ac} {self.unshard_cnt} {self._training_state}")
        repeated_skip = (
            self.repeated and self.unshard_cnt < 4 * self.num_repeat - 1 and is_frozen
        )
        # repeated_skip = False
        # logging.info(f"hkz {self.repeated} {self.unshard_cnt} {self.num_repeat} {self._training_state} {repeated_skip}")
        if self.time_dim_skip:
            size = self.flat_param.data.numel() // 8
            target_rank = (self.rank + 4) % 8
            self.partial_unshard = self.flat_param.data[
                size * target_rank : size * (target_rank + 1)
            ].clone()
        if self.ac_skip or repeated_skip:
            self.log_memory_trace()
            if self.profile:
                self.memory_state[f"end_reshard_{self._training_state}"] = (
                    torch.cuda.memory_allocated(self.device)
                )
            logging.info(
                f"(not resharded) after reshard {self.flat_param.device} {self.flat_param.requires_grad} {self.handle_init_id} {self._training_state} {torch.cuda.memory_allocated(self.device)} {self.unshard_cnt}"
            )
            # if self.debug:
            #     logging.info(
            #         f"(not resharded) after reshard {self.flat_param.device} {self.flat_param.requires_grad} {self.handle_init_id} {self._training_state} {torch.cuda.memory_allocated(self.device)}"
            #     )
            return  # no reshard, always replicate full parameters
        self._use_sharded_flat_param()
        if free_unsharded_flat_param:
            self._free_unsharded_flat_param()
        if self.debug:
            logging.info(
                f"after reshard {self.flat_param.device} {self.flat_param.requires_grad} {self.handle_init_id} {self._training_state} {torch.cuda.memory_allocated(self.device)}"
            )
        if self.profile:
            self.memory_state[f"end_reshard_{self._training_state}"] = (
                torch.cuda.memory_allocated(self.device)
    ...
    def log_memory_trace(self):
        if self.profile:
            FlatParamHandle.memory_traces.append(
                [torch.cuda.memory_allocated(self.device), time.time()]
            )

    def log_memory(self):
        if self.debug:
            logging.info(
                f"log memory {self.flat_param.device} {self.flat_param.requires_grad} {self.handle_init_id} {self._training_state} {torch.cuda.memory_allocated(self.device)}"
            )
