...
from fthub.fsdp._init_utils import HYBRID_SHARDING_STRATEGIES
from fthub.fsdp._utils import (
    _apply_to_tensors,
    _no_dispatch_record_stream,
    p_assert,
)
from fthub.fsdp.api import BackwardPrefetch
...
def _share_state_and_init_handle_attrs(
    ...
    for fsdp_state in traversal_utils._get_fsdp_states(root_module):
        ...
        fsdp_state._needs_pre_forward_unshard = (
            root_state._needs_pre_forward_unshard
        )  # bug fix
...
def _unshard(
    ...
    # hkz-comment: pre_unshard is about changing the precision or offloading
...
def _pre_forward(
    ...
    args = _register_post_backward_no_grad_hooks(state, module, args, handles)
    ...
    for handle in handles:
        handle.prof_memory_pre_forward()
    return args, kwargs
...
def _post_forward(
    ...
    for handle in handles:
        ...
        handle.prof_memory_post_forward()
...
def _pre_backward_hook(
    ...
    *unused: Any,
    ...
    if 1:
        ...
        # logging.info("hkz pre_backward_unshard")
        _unshard(
...
@no_type_check
@torch.no_grad()
def _post_backward_for_no_grad_hook(
    state: _FSDPState,
    handles: FlatParamHandle,
    *unused: Any,
):
    for handle in handles:
        if not isinstance(handle, FlatParamHandle):
            return
        already_resharded = (
...
def _post_backward_hook(
    ...
    *unused: Any,
    ...
    with torch.autograd.profiler.record_function(
        ...
        # be in `BACKWARD_POST`.
        # hkz-comment: Skip the following check for activation checkpointing
        # p_assert(
...
@no_type_check
def _register_post_backward_no_grad_hooks(
    state: _FSDPState,
    module: nn.Module,
    inputs: Any,
    handles: List[FlatParamHandle],
    ...
    def _register_hook(t: torch.Tensor) -> torch.Tensor:
        if t.requires_grad:
            t.register_hook(
                functools.partial(_post_backward_for_no_grad_hook, state, handles)
            )
        return t

    return _apply_to_tensors(_register_hook, inputs)
...
def _register_post_backward_hooks(
    ...
    for handle in handles:
        ...
        # hkz-comment: won't call post_backward for parameters that does not need grad
