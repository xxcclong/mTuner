...
from fthub.fsdp._dynamo_utils import _annotate_modules_for_dynamo
from fthub.fsdp._init_utils import (
...
from fthub.fsdp._wrap_utils import _auto_wrap
...
class FullyShardedDataParallel(nn.Module, _FSDPState):
    ...
    Args:
        ...
        auto_wrap_policy (Optional[Union[Callable[[nn.Module, bool, int], bool], _FSDPPolicy]]):
            ...
            policy. ``ModuleWrapPolicy`` in ``fthub.fsdp.wrap.py``
            ...
            ``fthub.fsdp.wrap.py`` gives an example callable that
    ...
    def __init__(
        ...
        torch._C._log_api_usage_once("fthub.fsdp")
        super().__init__()
        ...
        backward_prefetch_limit = 1000
        forward_prefetch_limit = 1000
        _init_core_state(
            self,
            sharding_strategy,
            mixed_precision,
        ...
        _init_replica_state(self, replica_config)
        _init_param_handle_from_module(
            self,
            module,
            device_id,
    ...
    def forward(self, *args: Any, **kwargs: Any) -> Any:
        ...
        if 1:
            ...
            # print(f"hkz begin forward {self._is_root} {self._fsdp_wrapped_module}")
            output = self._fsdp_wrapped_module(*args, **kwargs)
            # print(f"end forward {self}")
            # return output
    ...
    def optim_state_dict(
        ...
        Example::
            ...
            >>> # xdoctest: +SKIP("undefined variables")
            >>> from fthub.fsdp import FullyShardedDataParallel as FSDP
            >>> from fthub.fsdp import StateDictType
            >>> from fthub.fsdp import FullStateDictConfig
            >>> from fthub.fsdp import FullOptimStateDictConfig
            >>> # Save a checkpoint
            >>> model, optim = ...
            >>> FSDP.set_state_dict_type(
            >>>     model,
