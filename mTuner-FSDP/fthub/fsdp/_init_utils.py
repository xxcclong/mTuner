...
import fthub.fsdp._exec_order_utils as exec_order_utils
import fthub.fsdp._traversal_utils as traversal_utils
import fthub.fsdp.fully_sharded_data_parallel as fsdp_file
...
from fthub.fsdp._limiter_utils import _FreeEventQueue
from fthub.fsdp._wrap_utils import _get_fully_sharded_module_to_states
from fthub.fsdp.api import (
    BackwardPrefetch,
    CPUOffload,
...
from fthub.fsdp.flat_param import (
    _HandlesKey,
    FlatParameter,
    FlatParamHandle,
    HandleShardingStrategy,
)
from fthub.fsdp.wrap import _FSDPPolicy
...
class ReplicaConfig:
    def __init__(self, config):
        self.intra_weight_replica_rate = config.intra_weight.replica_rate
        # self.inter_weight_replica_rate = config.inter_weight.replica_rate
        self.time_dim_layer_id = config.time_dim.layer_id
        self.intra_weight_replica_strategy = config.intra_weight.replica_strategy
        self.config = config
    ...
    if process_group is None:
        ...
        if _is_valid_hybrid_shard_pg_type(process_group):
            ...
            raise ValueError(
                ...
                f"Tuple[dist.ProcessGroup, dist.ProcessGroup] but got {type(process_group)} {len(process_group)} {[type(item) for item in process_group]} {[item for item in process_group]}"
            )
...
def _init_core_state(
    ...
    _fully_sharded_module_to_handles: Dict[nn.Module, List[FlatParamHandle]] = (
...
def _init_replica_state(state: _FSDPState, replica_config: ReplicaConfig) -> _FSDPState:
...
def _init_param_handle_from_params(
    ...
    handle = FlatParamHandle(
        ...
        replica_config=state.replica_config,
    )
    ...
    for param_name, param in fsdp_module.named_parameters():
        if param not in ignored_params and not _is_fsdp_flattened(param):
            raise RuntimeError(
                ...
                f"{param.size()} {param.__class__} {param.device}"
            )
