...
for model_size in "${model_sizes[@]}"; do
  for seq in "${seq_lens[@]}"; do
    ...
    SEQ=$seq TP=$TP BS=$BS MODEL_SIZE=$model_size MTUNER=$MTUNER DISABLE_TP_COMM_OVERLAP=true \
    srun --exclusive --partition=debug --nodes=1 --gres=gpu:8 \
    bash examples/gpt3/train_gpt3_175b_distributed.sh \
    | tee megatron_output/result_modelsize${model_size}_seq${seq}.txt
  done
done