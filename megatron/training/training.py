...
def training_log(loss_dict, total_loss_dict, learning_rate, decoupled_learning_rate, iteration,
    ...
    names = timers._timers.keys()
    tmp = []
    for name in names:
        if "attn-layer" in name or "transformer-layer" in name or "hkz" in name:
            tmp.append(name)
    # Logging.
    timers_to_log = [
        'forward/decoder',
        ...
        'optimizer']
    timers_to_log = tmp + timers_to_log
    ...
    if iteration % args.log_interval == 0:
        ...
        log_string += ' TPS: {:.3f} |'.format(batch_size * args.seq_length / (elapsed_time_per_iteration))
        log_string += ' TGS: {:.3f} |'.format(batch_size * args.seq_length / torch.distributed.get_world_size() / (elapsed_time_per_iteration))
        total_loss_dict[advanced_iters_key] = 0
        ...
        if report_memory_flag and learning_rate > 0.:
            ...
            if torch.distributed.get_rank() == 0 and report_memory_flag:
                num_microbatches = get_num_microbatches()
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    log_name = f'logs/{datetime.now().strftime("%m%d-%H%M")}/rank_{torch.distributed.get_rank()}'
    ...
    while iteration < args.train_iters:
        if args.profile and torch.distributed.get_rank() in args.profile_ranks:
            ...
            if iteration == args.profile_step_start - 1:
                timers.set_profiling(True)
