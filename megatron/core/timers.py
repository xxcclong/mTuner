...
class Timer(TimerBase):
    ...
    def __init__(self, name, enable_prof):
        """Initialize Timer.
    ...
    def start(self, barrier=False):
        ...
        self._start_time = time.time()
        self._started = True
        if self._enable_prof:
            self.record = torch.autograd.profiler.record_function(name="SCH-" + self.name)
...
class Timers:
    ...
    def __init__(self, log_level, log_option):
        ...
        self.target_profiling_names = ["forward-compute", "backward-compute"]

    def set_profiling(self, is_profiling):
        self.is_profiling = is_profiling
        for name in self._timers:
            self._timers[name]._enable_prof = is_profiling and name in self.target_profiling_names
    ...
    def __call__(self, name, log_level=None):
        ...
        self._timers[name] = Timer(name, enable_prof=self.is_profiling and name in self.target_profiling_names)
