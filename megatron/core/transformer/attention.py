...
class Attention(MegatronModule, ABC):
    ...
    def __init__(
        ...
        # print(f"{submodules.core_attention=}")
        ...
        # self.buffer = torch.empty([1024 * 1024], dtype=torch.bfloat16, device="cuda")
        # print(f"linear_proj: {self.linear_proj.weight.shape} {self.linear_proj=}")
        # exit()
    ...
    def forward(
        ...
        """
        # self.config.timers('hkz-attention').start()
        ...
        # t0 = time.time()
        # self.config.timers('hkz-attention1').start()
        query, key, value = self.get_query_key_value_tensors(hidden_states, key_value_states)
        # self.config.timers('hkz-attention1').stop()
        # torch.cuda.synchronize()
        # t1 = time.time()
        ...
        # torch.cuda.synchronize()
        # t2 = time.time()
        # self.config.timers('hkz-rope').start()
        ...
        # self.config.timers('hkz-rope').stop()
        # ==================================
        # core attention computation
        # ==================================
        # torch.cuda.synchronize()
        # t3 = time.time()
        if self.config.use_mtuner:
            layer_id = self.config.curr_layer[0]
            self.config.curr_layer[0] += 1
            do_weight_gather = (layer_id < len(self.config.base_weights) - 1 and layer_id >= self.config.num_offset - 1)
        else:
            do_weight_gather = False
        # self.config.events1[layer_id].record()
        if do_weight_gather:
            with torch.cuda.stream(self.config.stream):

                # self.config.target_weights[layer_id + 1].weight._typed_storage()._resize_(self.config.total_size)
                # torch.distributed.all_gather_into_tensor(self.config.target_weights[layer_id + 1].weight.data, self.config.base_weights[layer_id + 1], group=get_data_parallel_group())
                # if layer_id - 1 > self.config.num_offset - 1:
                #     self.config.target_weights[layer_id - 1].weight._typed_storage()._resize_(0)

                torch.distributed.all_gather_into_tensor(self.config.buffer, self.config.base_weights[layer_id + 1], group=self.config.attn_group, async_op=True)
                        # self.config.buffer.copy_(self.config.buffer2)
                        # torch.cuda.synchronize()
                    # self.config.buffer2.add_(1)
                    # assert not self.config.buffer2.requires_grad
                #     self.config.target_weights[layer_id + 1].weight.data = self.config.buffer2
                # else:
                #     if layer_id in list(range(5,70)):
                #         self.config.target_weights[layer_id + 1].weight.data = self.config.buffer2
                #     else:
                #         self.config.target_weights[layer_id + 1].weight.data = self.config.buffer
            # self.config.events2[layer_id].record()
        if self.checkpoint_core_attention and self.training:
            ...
            def hook(grad, config):
                layer_id = config.curr_layer[0]
                config.curr_layer[0] -= 1
                if layer_id <= config.num_offset: # layer 0 no opt
                    return grad
                with torch.cuda.stream(config.stream):
                # if 1:
                    # config.target_weights[layer_id - 1].weight._typed_storage()._resize_(config.total_size)
                    # torch.distributed.all_gather_into_tensor(config.target_weights[layer_id - 1].weight.data, config.base_weights[layer_id - 1], group=get_data_parallel_group())
                    # if layer_id + 1 <= len(config.base_weights) - 1:
                    #     config.target_weights[layer_id + 1].weight._typed_storage()._resize_(0)

                    torch.distributed.all_gather_into_tensor(config.buffer, config.base_weights[layer_id - 1], group=config.attn_group, async_op=True)
                    config.target_weights[layer_id - 1].weight.data = config.buffer
                return grad
            if self.config.use_mtuner:
                core_attn_out.register_hook(partial(hook, config=self.config))
        # torch.cuda.synchronize()
        # t4 = time.time()
        # torch.cuda.current_stream().wait_event(self.config.events2[30])
        ...
        # self.config.timers('hkz-attention').stop()
        # arr = [t0, t1, t2, t3, t4, t5]
        # times = [arr[1:][i] - arr[:-1][i] for i in range(len(arr) - 1)]
        # print(f"{times=}")
        # print(f"{torch.distributed.get_rank()=} {output.device=} {self.linear_proj=}")
        return output, bias
...
class SelfAttention(Attention):
    ...
    def __init__(
        ...
        # print(f"linear_qkv: {self.linear_qkv.weight.shape}")
...
class CrossAttention(Attention):
    ...
    def __init__(
        ...
        # print(f"linear_kv: {self.linear_kv.weight.shape} linear_q: {self.linear_q.weight.shape}")
