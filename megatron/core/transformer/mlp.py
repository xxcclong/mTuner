...
class MLP(MegatronModule):
    ...
    def __init__(
        ...
        if self.config.gated_linear_unit:
            ffn_hidden_size *= 2
            # print(f"hkz 2")
        # print(f"ffn_hidden_size {ffn_hidden_size} input_size {self.input_size} hidden_size {self.config.hidden_size}")
        # print(f"seq {self.config.sequence_parallel}")
        ...
        # print(f"linear_fc1 {self.linear_fc1.weight.shape} linear_fc2 {self.linear_fc2.weight.shape}")

    def forward(self, hidden_states):

        # [s, b, 4 * h/p]
        # self.config.timers('hkz-forward_mlp').start()
        # print(f"{hidden_states.shape=}")
        # torch.cuda.synchronize()
        # t0 = time.time()
        # self.config.timers('hkz-forward_mlp1').start()
        intermediate_parallel, bias_parallel = self.linear_fc1(hidden_states)
        # self.config.timers('hkz-forward_mlp1').stop()
        bias_parallel = None
        ...
        # [s, b, h]
        # print(f"interm2 {intermediate_parallel.shape}")
        output, output_bias = self.linear_fc2(intermediate_parallel)
        output_bias = None
        # torch.cuda.synchronize()
        # t1 = time.time()
        # print(f"{t1 - t0=}")
        # print(f"interm3 {output.shape}")
        # self.config.timers('hkz-forward_mlp').stop()
