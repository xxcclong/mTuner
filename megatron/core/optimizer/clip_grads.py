...
def get_grad_norm_fp32(
    ...
    # print(f"a {torch.cuda.memory_allocated()=}")
    # Calculate norm.
    if norm_type == inf:
        ...
        # print(f"a1 {torch.cuda.memory_allocated()=}")
        if norm_type == 2.0:
        ...
        # print(f"a2 {torch.cuda.memory_allocated()=} {torch.cuda.memory_reserved()=} {total_norm.shape=}")
        # Sum across all model-parallel GPUs.
        torch.distributed.all_reduce(
            total_norm, op=torch.distributed.ReduceOp.SUM, group=model_parallel_group
        )
        # print(f"a3 {torch.cuda.memory_allocated()=}")
        total_norm = total_norm.item() ** (1.0 / norm_type)

    # print(f"b {torch.cuda.memory_allocated()=}")
